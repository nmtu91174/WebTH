...
namespace WebTH.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        private string GenerateUniqueOrderCode()
        {
            ...
            do
            {
                code = "DH" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + Guid.NewGuid().ToString("N").Substring(0, 4).ToUpper();
            }
        ...
        //    // ✅ Thêm Include để tránh lazy load crash
        ...
        //    // ✅ Lấy ảnh từ ProductImage trước, fallback về cột Image
        //    var defaultImg = checkProduct.ProductImage?.FirstOrDefault(x => x.IsDefault);
        ...
        //    {
        //        code = new { Success = false, msg = "Sản phẩm không đủ số lượng trong kho!", code = -1, Count = cart.Items.Count };
        //    }
        //    else
        //    {
        ...
        public ActionResult AddToCart(int id, int quantity)
        {
            ...
            if (checkProduct != null)
            {
                ...
                // ✅ Lấy ảnh từ ProductImage trước, fallback về cột Image
                var defaultImg = checkProduct.ProductImage?.FirstOrDefault(x => x.IsDefault);
                if (defaultImg != null && !string.IsNullOrEmpty(defaultImg.Image))
                {
                    item.ProductImg = defaultImg.Image;
                }
                else if (!string.IsNullOrEmpty(checkProduct.Image))
                {
                    // Sản phẩm mới chỉ có cột Image
                    item.ProductImg = checkProduct.Image.StartsWith("/")
                        ? checkProduct.Image
                        : "/Uploads/images/sanpham/" + checkProduct.Image;
                }
                ...
                if (item.Quantity > checkProduct.Quantity)
                {
                    code = new { Success = false, msg = "Sản phẩm trong kho không đủ! Còn lại " + checkProduct.Quantity, code = -1, Count = cart.Items.Count };
                }
                else
                {
                    cart.AddToCart(item, quantity);
            ...
            {
                code = new { Success = false, msg = "Không tìm thấy sản phẩm!", code = -1, Count = 0 };
            }
